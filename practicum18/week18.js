...
function makeTwo() {
	//Ваш код
	window.localStorage.setItem("keyTwo", "7,6,5");
...
function makeThree() {
	//Ваш код
	window.localStorage.setItem("keyThree", "name=John, age=25");
...
function makeFour() {
	//Ваш код
	window.localStorage.setItem("keyFour", true);
...
function makeFive() {
	//Ваш код
	window.localStorage.setItem("keyFive", null);
...
function makeSeven() {
	...
	window.localStorage.setItem("objectSeven", objectJSON);
	console.log('Объект obj был записан в Local Storage.');
}
...
const showNine = () => {
	...
	// const resultFourteen = JSON.parse(arrayNineString);
	if (arrayNineString) {
		//Ваш код
...
const showSeven = () => {
	...
	if (objectSevenString) {
		...
		for (const key in objectSeven) {
			if (key === "city") {
				let city = objectSeven[key];
			}
			if (key === "population") {
				const population = objectSeven[key];
			}

		} result.innerHTML = `В ${city}е живёт ${population} человек`;
